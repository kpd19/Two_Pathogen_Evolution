...
parameters {
  vector<lower=0.00000001,upper=0.1>[I] nu_bar_S;  	// no
  matrix<lower=0.00000001,upper=5>[I,T] C_S;  		// tree
  real<lower = 0, upper = 0.1> mu_S;			// no
  vector<lower = 0, upper = 5>[T] muC_S;		// tree
  real<lower = 0, upper = 4> sigma_S;			// no
  vector<lower = 0, upper = 5>[T] sigmaC_S;		// tree
...
transformed parameters {
  ...
  for(n in 1:N){
    if (morph[n] == 1) // if multi-capsid, C does not vary by tree, nu_bar does 
      prob[n] = 1 - (1 + nu_bar_M[isolate[n]]* C_M[isolate[n],trees[n]]^2 * pathogen[n] * 7.0*0.032)^(-1/C_M[isolate[n],trees[n]]^2); 	// p_infection
    else // if single-capsid, C does not vary by tree, nu_bar does 
      prob[n] = 1 - (1 + nu_bar_S[isolate[n]]* C_S[isolate[n],trees[n]]^2 * pathogen[n] * 7.0*0.032)^(-1/C_S[isolate[n],trees[n]]^2); 	// p_infection
  }
...
model {
  ...
  mu_S ~ normal(0.001,0.01);
  sigma_S ~ lognormal(-3,1);
  ...
  for(i in 1:I){
    nu_bar_S[i] ~ normal(mu_S,sigma_S);
    nu_bar_M[i] ~ normal(mu_M,sigma_M);
  }
  
  for(t in 1:T) {  
    for(i in 1:I){
      C_S[i,t] ~ normal(muC_S[t],sigmaC_S[t]);
