...
transformed parameters {
  ...
  for(n in 1:N){
    if (morph[n] == 1) // if multi-capsid, C does not vary by tree, nu_bar does 
      prob[n] = 1 - (1 + nu_bar_M[isolate[n],trees[n]]* C_M[isolate[n]]^2 * pathogen[n] * 7.0*0.032)^(-1/C_M[isolate[n]]^2); 	// p_infection
    else // if single-capsid, C varies by tree, nu_bar does not
