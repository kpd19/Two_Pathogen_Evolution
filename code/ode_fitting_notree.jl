...
function run_simulation(d, pdoug, eps_val, gen, phi1, phi2, νSDO, νSGR, νMDO, νMGR, CSDO, CSGR, CMDO, CMGR,rep, tree_vals, si,sr,rho,sigma,bi,br,gamma,r)
    ...
    sall = mean(snpv_frac[2:201])
    mall = mean(mnpv_frac[2:201])
    ...
    for i in 1:length(small_df.id)
        ...
        probs_list = [ldbinom(small_df.MNPV[i],small_df.total[i],m10), ldbinom(small_df.MNPV[i],small_df.total[i],m30),ldbinom(small_df.MNPV[i],small_df.total[i],mall)]
...
sr_test = 0.5:.25:4
si_test = 5:2.5:40
#length(sr_test)*length(si_test)*100/1080*90/60
...
for srt in sr_test
    ...
    sim_info = "sM"*string(srt)*"stoch"*string(idx)*".csv"
    #sim_info = "douglas_test.csv"
    #CSV.write(dir*"info_"*sim_info, sim_data, header = true)
    CSV.write(dir*"LL_"*sim_info, summary_data, header = true)
